...
#include <fbxsdk/core/fbxclassid.h>
											...
											cVector3f vParentT, cVector3f vParentS, cVector3f vParentR, cSkeleton * apSkeleton)
	{
		...
		auto* pAnimationStack = FbxCast<FbxAnimStack>(apScene->FindMember(FbxCriteria::ObjectType(FbxAnimStack::ClassId), asAnimStackName.c_str()));
		auto* pAnimLayer = FbxCast<FbxAnimLayer>(pAnimationStack->GetMember(FbxCriteria::ObjectType(FbxAnimLayer::ClassId), 0));
		...
		if( pAnimationStack != NULL && apNode->GetSkeleton() )
		{
			...
			for(;it != setTimes.end(); it++)
			{
				...
				vTempKeyFrame[i].qFinalRot = cQuaternion( pRotation.GetAt(3), pRotation[0], pRotation[1], pRotation[2] );
	...
	void cMeshLoaderFBX::LoadMeshData(tSubMeshDataList* apSubMeshList,cSkeleton* apSkeleton, cNode3D* apHplNode, FbxNode *apNode, int alDepth, bool animationOnly)
	{
		...
		if ( !animationOnly && !isControllerNode )    // exclude controller objects, used only for animation, don't contain mesh data
		{
			...
			if(mbTriangulated==false) {
			    assert(false);
			    // pTriMesh = pConverter.Triangulate(pMesh, true, true);
			}
	
			//mpSdkManager->DestroyFbxGeometryConverter(pConverter);
			...
			if(pMaterialLayer)
			{
				...
				FbxSurfaceMaterial *pMat = pMatArray[0];
	...
	{
		switch(alNum.GetOrder())
		{
		case FbxEuler::eOrderXYZ: return "X Y Z";
		case FbxEuler::eOrderXZY: return "X Z Y";
		case FbxEuler::eOrderYZX: return "Y Z X";
		case FbxEuler::eOrderYXZ: return "Y X Z";
		case FbxEuler::eOrderZXY: return "Z X Y";
		case FbxEuler::eOrderZYX:return "Z Y X";
		case FbxEuler::eOrderSphericXYZ: return "SphereXYZ";
		}
		return "Unknown";
	}
